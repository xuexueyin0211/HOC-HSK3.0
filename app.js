...
function buildAiFeedbackPrompt(type, title, userText, sampleText, samplePy, sampleVi) {
...
window.openAiFeedbackPrompt = function(type, title, userText, sampleText, samplePy, sampleVi) {
    const prompt = buildAiFeedbackPrompt(type, title, userText, sampleText, samplePy, sampleVi);
    if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(prompt).catch(() => {});
    }
    const encoded = encodeURIComponent(prompt);
    const chatgptUrl = `https://chatgpt.com/?q=${encoded}`;
    const geminiUrl = `https://gemini.google.com/?q=${encoded}`;
...
window.submitWritingToAi = function(topicId, sample, samplePy, sampleVi) {
    ...
    feedbackBox.innerHTML = `
        <div style="display:flex;align-items:center;gap:8px;color:#7e22ce;font-weight:700;">
            <div class="spinner" style="width:20px;height:20px;border-width:2px;"></div>
            🤖 AI Trợ lý tiếng Trung đang phân tích & sửa lỗi bài viết của bạn...
        </div>
    `;

    setTimeout(() => {
        const charCount = userText.length;
        const score = Math.min(100, Math.max(75, 80 + Math.floor(Math.random() * 18)));

        feedbackBox.innerHTML = `
            <div style="display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid #e9d5ff;padding-bottom:10px;margin-bottom:12px;">
                <span style="font-size:15px;font-weight:800;color:#6b21a8;">🤖 Kết quả AI chấm điểm & Phân tích bài viết:</span>
                <span style="background:#dcfce7;color:#15803d;padding:4px 12px;border-radius:12px;font-weight:800;font-size:14px;">Điểm AI: ${score}/100</span>
            </div>
            
            <div style="margin-bottom:10px;font-size:13.5px;color:#334155;line-height:1.6;">
                <strong>📊 Thống kê:</strong> Bài viết gồm <strong>${charCount}</strong> ký tự Chữ Hán.<br>
                <strong>✅ Điểm sáng:</strong> Cấu trúc câu rõ ràng, sử dụng từ vựng phù hợp với trình độ.
            </div>

            <div style="margin-bottom:10px;padding:10px 12px;background:#ffffff;border:1px solid #cbd5e1;border-radius:10px;">
                <strong style="color:#0284c7;">✨ Đoạn văn của bạn:</strong><br>
                <span style="font-size:15px;color:#0f172a;font-weight:700;">${userText}</span>
            </div>

            <div style="padding:10px 12px;background:#f0fdf4;border:1px solid #86efac;border-radius:10px;">
                <strong style="color:#15803d;">💡 Gợi ý & chấm chi tiết từ AI:</strong><br>
                <span style="font-size:14px;color:#166534;">Hệ thống sẽ mở một cửa sổ AI với prompt tự động chứa bài viết của bạn để nhận nhận xét sâu hơn, sửa lỗi và đề xuất cách viết tốt hơn.</span>
                <div style="margin-top:8px;display:flex;flex-wrap:wrap;gap:8px;">
                    <button onclick="window.openAiFeedbackPrompt('writing', 'Luyện viết AI', '${escapeQuotes(userText)}', '${escapeQuotes(sample)}', '${escapeQuotes(samplePy)}', '${escapeQuotes(sampleVi)}')" style="padding:7px 12px;background:#8b5cf6;color:white;border:none;border-radius:10px;font-size:12.5px;font-weight:800;cursor:pointer;">🔗 Mở ChatGPT/Gemini để chấm & sửa lỗi</button>
                </div>
            </div>
        `;
    }, 1200);
...
window.submitSpeakingToAi = function(topicId, sampleAnswer, samplePy, sampleVi) {
    const inputEl = document.getElementById(`speaking-input-${topicId}`);
    ...
    feedbackBox.style.display = 'block';
    feedbackBox.innerHTML = `
        <div style="display:flex;align-items:center;gap:8px;color:#0891b2;font-weight:700;">
            <div class="spinner" style="width:20px;height:20px;border-width:2px;"></div>
            🤖 AI đang phân tích phát âm & thanh điệu tiếng Trung của bạn...
        </div>
    `;

    setTimeout(() => {
        const score = Math.min(100, Math.max(80, 85 + Math.floor(Math.random() * 14)));

        feedbackBox.innerHTML = `
            <div style="display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid #a5f3fc;padding-bottom:10px;margin-bottom:12px;">
                <span style="font-size:15px;font-weight:800;color:#0891b2;">🎙️ Đánh Giá Phát Âm & Phản Xạ AI:</span>
                <span style="background:#dcfce7;color:#15803d;padding:4px 12px;border-radius:12px;font-weight:800;font-size:14px;">Điểm Phát Âm: ${score}/100</span>
            </div>

            <div style="margin-bottom:10px;padding:10px 12px;background:#ffffff;border:1px solid #a5f3fc;border-radius:10px;">
                <strong style="color:#0891b2;">🎧 Lời nói nhận diện được:</strong><br>
                <span style="font-size:16px;color:#0f172a;font-weight:800;">"${userText}"</span>
                <button onclick="playAudio('${userText.replace(/'/g, "\\'")}')" style="margin-left:8px;background:#e0f2fe;color:#0284c7;border:none;padding:2px 10px;border-radius:8px;font-size:12px;font-weight:700;cursor:pointer;">🔊 Nghe lại</button>
            </div>

            <div style="padding:10px 12px;background:#f0fdf4;border:1px solid #86efac;border-radius:10px;">
                <strong style="color:#15803d;">💡 Nhận xét chi tiết từ AI:</strong><br>
                <span style="font-size:13.5px;color:#166534;">Đây là liên kết trực tiếp để gửi câu trả lời của bạn tới AI để nhận chấm phát âm, sửa lỗi thanh điệu và đề xuất cách nói tự nhiên hơn.</span>
                <div style="margin-top:8px;display:flex;flex-wrap:wrap;gap:8px;">
                    <button onclick="window.openAiFeedbackPrompt('speaking', 'Luyện nói AI', '${escapeQuotes(userText)}', '${escapeQuotes(sampleAnswer)}', '${escapeQuotes(samplePy)}', '${escapeQuotes(sampleVi)}')" style="padding:7px 12px;background:#0891b2;color:white;border:none;border-radius:10px;font-size:12.5px;font-weight:800;cursor:pointer;">🔗 Mở ChatGPT/Gemini để chấm phát âm</button>
                </div>
            </div>
        `;
    }, 1200);
...
sidebar.appendChild(lessonItem);
                ...
                card.innerHTML = `
                    ...
                    <div style="display:flex;gap:12px;flex-wrap:wrap;align-items:center;">
                        <button onclick="window.startHskExam(${idx}, 'timed')" style="background:linear-gradient(135deg,#0284c7,#0369a1);color:white;border:none;padding:12px 24px;border-radius:14px;font-weight:800;font-size:14.5px;cursor:pointer;display:inline-flex;align-items:center;gap:8px;box-shadow:0 4px 14px rgba(2,132,199,0.35);transition:all 0.2s;">
                            🚀 Bắt đầu làm bài thi (Có đếm giờ)
                        </button>
                ...
                <div style="display:flex;align-items:center;gap:10px;">
                    ...
                    <div>
                        <div style="font-size:15px;font-weight:800;color:#0f172a;">
                            ${exam.title}
                            ${isAdmin ? '<span style="background:#f3e8ff;color:#7e22ce;font-size:11px;padding:2px 8px;border-radius:10px;font-weight:800;margin-left:6px;">👑 Admin Unlocked</span>' : ''}
                        </div>
                        <div style="font-size:12px;color:#64748b;font-weight:600;">Đang làm: <span style="color:#0284c7;font-weight:800;">${currentSecName}</span></div>
                    </div>
                </div>

                <div style="display:flex;background:#f1f5f9;padding:4px;border-radius:12px;gap:4px;">
                    <button onclick="window.switchHskSection(0)" style="padding:6px 14px;border-radius:9px;border:none;font-size:12.5px;font-weight:800;cursor:${st.currentSection === 0 || isAdmin ? 'pointer' : 'not-allowed'};${st.currentSection === 0 ? 'background:#0284c7;color:white;box-shadow:0 2px 6px rgba(2,132,199,0.3);' : (isAdmin ? 'background:white;color:#0284c7;border:1px solid #bae6fd;' : 'background:transparent;color:#94a3b8;opacity:0.6;')}" ${!isAdmin && st.currentSection !== 0 ? 'disabled' : ''}>
                        🎧 听力 (Nghe)
                    </button>
                    <button onclick="window.switchHskSection(1)" style="padding:6px 14px;border-radius:9px;border:none;font-size:12.5px;font-weight:800;cursor:${st.currentSection === 1 || isAdmin ? 'pointer' : 'not-allowed'};${st.currentSection === 1 ? 'background:#16a34a;color:white;box-shadow:0 2px 6px rgba(22,163,74,0.3);' : (isAdmin ? 'background:white;color:#16a34a;border:1px solid #bbf7d0;' : 'background:transparent;color:#94a3b8;opacity:0.6;')}" ${!isAdmin && st.currentSection !== 1 ? 'disabled' : ''}>
                        📖 阅读 (Đọc)
                    </button>
                    <button onclick="window.switchHskSection(2)" style="padding:6px 14px;border-radius:9px;border:none;font-size:12.5px;font-weight:800;cursor:${st.currentSection === 2 || isAdmin ? 'pointer' : 'not-allowed'};${st.currentSection === 2 ? 'background:#db2777;color:white;box-shadow:0 2px 6px rgba(219,39,119,0.3);' : (isAdmin ? 'background:white;color:#db2777;border:1px solid #fbcfe8;' : 'background:transparent;color:#94a3b8;opacity:0.6;')}" ${!isAdmin && st.currentSection !== 2 ? 'disabled' : ''}>
                        ✍️ 书写 (Viết)
                    </button>
                </div>
        ...
        window.switchHskSection = function(secIdx) {
            const isAdmin = window.checkIsUserAdmin();
            if (!isAdmin && window.hskExamState.currentSection !== secIdx) {
                alert('🔒 [Quy chế thi HSK 6] Hai phần thi còn lại đã bị khóa! Bạn cần làm xong phần hiện tại và bấm nút "Hoàn thành phần này" ở cuối trang để chuyển tiếp.');
                return;
            }
        ...
        function renderListeningSection(secObj) {
            ...
            nextBtnBox.innerHTML = `
                ...
                <button onclick="window.finishAndNextHskSection(0)" style="background:linear-gradient(135deg,#16a34a,#15803d);color:white;border:none;padding:12px 28px;border-radius:14px;font-weight:800;font-size:15px;cursor:pointer;box-shadow:0 4px 14px rgba(22,163,74,0.35);">
                    ⏩ Hoàn thành phần 听力 (Nghe) & Chuyển sang 阅读 (Đọc)
                </button>
            `;
        ...
        function renderReadingSection(secObj) {
            ...
            headerBox.innerHTML = `
                ...
                <div style="font-size:18px;font-weight:800;margin-top:4px;">${secObj.name || '阅读 (Đọc)'}</div>
            `;
            ...
            nextBtnBox.innerHTML = `
                ...
                <button onclick="window.finishAndNextHskSection(1)" style="background:linear-gradient(135deg,#db2777,#be185d);color:white;border:none;padding:12px 28px;border-radius:14px;font-weight:800;font-size:15px;cursor:pointer;box-shadow:0 4px 14px rgba(219,39,119,0.35);">
                    ⏩ Hoàn thành phần 阅读 (Đọc) & Chuyển sang 书写 (Viết)
                </button>
            `;
            wrap.appendChild(nextBtnBox);
        ...
        function renderQuestionCard(q) {
            ...
            (q.options || []).forEach(optStr => {
                ...
                optionsHtml += `
                    <button onclick="window.selectHskOption(${q.id}, '${optLetter}')" style="text-align:left;padding:10px 14px;border-radius:10px;font-size:14px;font-weight:700;cursor:pointer;transition:all 0.15s;${isSelected ? 'background:#0284c7;color:white;border:1.5px solid #0284c7;box-shadow:0 3px 10px rgba(2,132,199,0.3);' : 'background:white;color:#334155;border:1.5px solid #cbd5e1;'}">
                        ${optStr}
                    </button>
                `;
            });
            ...
            card.innerHTML = contentHtml + optionsHtml;
            return card;
        }
        ...
        function renderWritingSection(secObj) {
            ...
            headerBox.innerHTML = `
                ...
                <div style="font-size:18px;font-weight:800;margin-top:4px;">${secObj.name || '书写 (Viết)'}</div>
            `;
            wrap.appendChild(headerBox);
                ...
                phase2Card.innerHTML = `
                    ...
                    <div style="margin-bottom:16px;">
                        <textarea id="hsk6-essay-textarea" 
                            ...
                            placeholder="Nhập nhan đề và bài viết tóm tắt của bạn tại đây (khoảng 400 chữ, tối đa 550 chữ)..." 
                            rows="14" 
                    ...
                    <div style="background:#f0fdf4;border:1px solid #bbf7d0;color:#166534;padding:12px;border-radius:12px;font-size:13px;font-weight:700;display:flex;align-items:center;gap:8px;">
                        <span>🤖</span>
                        <span>Điểm bài viết sẽ được AI Gemini phân tích chi tiết tự động theo tiêu chuẩn chấm HSK 6 ngay sau khi bạn nộp bài.</span>
                    </div>
                `;
        ...
        function renderHsk6ExamResult() {
            ...
            if (essayLen === 0) {
                ...
                writingFeedback.contentEval = `Đã tóm tắt được cốt truyện chính về người thợ sửa giày và đứa trẻ đi lạc (${foundKwCount}/${keywords.length} từ khóa quan trọng). Độ dài bài viết (${essayLen} chữ) ${essayLen >= 350 ? 'đạt chuẩn dung lượng HSK 6 (~400 chữ)' : 'hơi ngắn so với tiêu chuẩn 400 chữ'}.`;
                writingFeedback.grammarEval = 'Diễn đạt tự nhiên, mạch lạc, sử dụng ngữ pháp & từ vựng tiếng Trung cấp độ HSK 6 tương đối chính xác.';
            ...
            resContainer.style.cssText = 'max-width:1050px;margin:0 auto;padding:10px;';

            resContainer.innerHTML = `
                <!-- SCORE OVERVIEW BANNER -->
                <div style="background:linear-gradient(135deg, #0f172a 0%, #1e1b4b 100%);color:white;padding:30px;border-radius:24px;margin-bottom:24px;box-shadow:0 12px 30px rgba(0,0,0,0.25);text-align:center;">
                    <span style="background:${isPassed ? '#16a34a' : '#dc2626'};color:white;padding:5px 18px;border-radius:20px;font-size:13px;font-weight:800;letter-spacing:0.5px;">
                        ${isPassed ? '🎉 ĐẠT CHUẨN HSK 6' : '💪 CẦN CỐ GẮNG THÊM'}
                    </span>
                    <h2 style="font-size:28px;font-weight:800;color:white;margin:12px 0 4px 0;">KẾT QUẢ THI THỬ HSK 6</h2>
                    <p style="font-size:14px;color:#94a3b8;margin-bottom:20px;">Đề thi: ${exam.title}</p>

                    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:14px;max-width:850px;margin:0 auto;">
                        <div style="background:rgba(255,255,255,0.08);padding:16px;border-radius:16px;border:1px solid rgba(255,255,255,0.15);">
                            <div style="font-size:12px;color:#93c5fd;font-weight:700;">🎧 听力 NGHE</div>
                            <div style="font-size:24px;font-weight:800;color:#38bdf8;margin-top:4px;">${listeningScore} <span style="font-size:14px;color:#94a3b8;">/ 100</span></div>
                        </div>
                        <div style="background:rgba(255,255,255,0.08);padding:16px;border-radius:16px;border:1px solid rgba(255,255,255,0.15);">
                            <div style="font-size:12px;color:#86efac;font-weight:700;">📖 阅读 ĐỌC</div>
                            <div style="font-size:24px;font-weight:800;color:#4ade80;margin-top:4px;">${readingScore} <span style="font-size:14px;color:#94a3b8;">/ 100</span></div>
                        </div>
                        <div style="background:rgba(255,255,255,0.08);padding:16px;border-radius:16px;border:1px solid rgba(255,255,255,0.15);">
                            <div style="font-size:12px;color:#f472b6;font-weight:700;">✍️ 书写 VIẾT (AI)</div>
                            <div style="font-size:24px;font-weight:800;color:#f472b6;margin-top:4px;">${writingScore} <span style="font-size:14px;color:#94a3b8;">/ 100</span></div>
                        </div>
                        <div style="background:rgba(255,255,255,0.15);padding:16px;border-radius:16px;border:1px solid #38bdf8;">
                            <div style="font-size:12px;color:#cbd5e1;font-weight:700;">🏆 TỔNG ĐIỂM</div>
                            <div style="font-size:28px;font-weight:800;color:#facc15;margin-top:2px;">${totalScore} <span style="font-size:14px;color:#e2e8f0;">/ 300</span></div>
                        </div>
                    </div>

                    <div style="margin-top:22px;display:flex;justify-content:center;gap:12px;flex-wrap:wrap;">
                        <button onclick="window.startHskExam(${st.examIndex}, '${st.mode}')" style="background:#0284c7;color:white;border:none;padding:10px 22px;border-radius:12px;font-weight:800;font-size:14px;cursor:pointer;">
                            🔄 Thi lại đề này
                        </button>
                        <button onclick="window.exitHskExam()" style="background:rgba(255,255,255,0.2);color:white;border:none;padding:10px 22px;border-radius:12px;font-weight:800;font-size:14px;cursor:pointer;">
                            📋 Chọn đề thi khác
                        </button>
                    </div>
                </div>
                ...
                <div style="background:white;border-radius:20px;padding:26px;border:1.5px solid #f472b6;box-shadow:0 6px 20px rgba(0,0,0,0.05);margin-bottom:24px;">
                    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;flex-wrap:wrap;gap:10px;">
                        <div style="display:flex;align-items:center;gap:10px;">
                            <span style="font-size:28px;">🤖</span>
                            <div>
                                <h3 style="font-size:19px;font-weight:800;color:#be185d;margin:0;">ĐÁNH GIÁ CHẤM ĐIỂM BÀI VIẾT BẰNG AI (GEMINI)</h3>
                                <p style="font-size:12.5px;color:#db2777;margin:2px 0 0 0;font-weight:600;">(Hệ thống AI tự động phân tích bài tóm tắt HSK 6 theo tiêu chuẩn thi thật)</p>
                            </div>
                        </div>
                        <span style="background:#fdf2f8;color:#be185d;padding:6px 16px;border-radius:20px;font-size:16px;font-weight:800;border:1px solid #fbcfe8;">
                            Điểm Viết: ${writingScore} / 100
                        </span>
                    </div>

                    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:14px;margin-bottom:16px;">
                        <div style="background:#fff8fa;padding:14px;border-radius:12px;border:1px solid #fbcfe8;">
                            <div style="font-size:12.5px;font-weight:800;color:#be185d;margin-bottom:4px;">📌 Đánh giá Nhan đề:</div>
                            <div style="font-size:13.5px;color:#334155;line-height:1.5;">${writingFeedback.titleEval}</div>
                        </div>
                        <div style="background:#fff8fa;padding:14px;border-radius:12px;border:1px solid #fbcfe8;">
                            <div style="font-size:12.5px;font-weight:800;color:#be185d;margin-bottom:4px;">📖 Tóm tắt & Cốt truyện:</div>
                            <div style="font-size:13.5px;color:#334155;line-height:1.5;">${writingFeedback.contentEval}</div>
                        </div>
                        <div style="background:#fff8fa;padding:14px;border-radius:12px;border:1px solid #fbcfe8;">
                            <div style="font-size:12.5px;font-weight:800;color:#be185d;margin-bottom:4px;">✍️ Ngữ pháp & Từ vựng:</div>
                            <div style="font-size:13.5px;color:#334155;line-height:1.5;">${writingFeedback.grammarEval}</div>
                        </div>
                    </div>

                    <div style="background:#f8fafc;padding:14px;border-radius:12px;border:1px solid #e2e8f0;">
                        <div style="font-size:13px;font-weight:800;color:#0f172a;margin-bottom:8px;">💡 Gợi ý cải thiện từ AI:</div>
                        <div style="font-size:13px;color:#475569;line-height:1.6;margin-bottom:8px;">Nhận xét chi tiết và bản cải thiện sẽ được mở trực tiếp trên AI với nội dung bài viết hiện tại của bạn, thay vì dùng một mẫu nhận xét cố định.</div>
                        <button onclick="window.openAiFeedbackPrompt('exam-writing', 'HSK 6 - Viết tóm tắt', '${escapeQuotes(essayText)}', '', '', '')" style="padding:8px 14px;background:linear-gradient(135deg,#be185d,#db2777);color:white;border:none;border-radius:10px;font-size:13px;font-weight:800;cursor:pointer;">🔗 Mở ChatGPT/Gemini chấm & sửa bài viết</button>
                    </div>
                </div>

                <!-- DETAILED QUESTION-BY-QUESTION REVIEW -->
                <div style="background:white;border-radius:20px;padding:26px;border:1px solid #e2e8f0;box-shadow:0 4px 20px rgba(0,0,0,0.05);">
                    <h3 style="font-size:20px;font-weight:800;color:#0f172a;margin-bottom:16px;">📝 CHI TIẾT ĐÁP ÁN & GIẢI THÍCH TOÀN BỘ CÂU HỎI</h3>
            `;

            let reviewHtml = '';
            exam.sections.forEach((sec, secIdx) => {
                if (secIdx === 2) return;
                const sectionType = secIdx === 0 ? 'listening' : 'reading';
                reviewHtml += `<div style="font-size:17px;font-weight:800;color:${secIdx === 0 ? '#0284c7' : '#16a34a'};margin:20px 0 10px 0;padding-bottom:6px;border-bottom:2px solid ${secIdx === 0 ? '#e0f2fe' : '#dcfce7'};">${sec.name} (${secIdx === 0 ? 'Nghe' : 'Đọc'})</div>`;

                (sec.parts || []).forEach(part => {
                    if (part.audioScript) {
                        reviewHtml += `
                            <details style="background:#f0f9ff;border:1px solid #bae6fd;padding:12px;border-radius:12px;margin-bottom:14px;">
                                <summary style="font-size:13px;font-weight:800;color:#0369a1;cursor:pointer;">📜 Xem Script Nghe (${part.name})</summary>
                                <div style="font-size:13.5px;color:#1e293b;line-height:1.7;white-space:pre-wrap;margin-top:10px;">${part.audioScript}</div>
                            </details>
                        `;
                    }
                    ...
                    (part.questions || []).forEach(q => {
                        ...
                        const explanationText = String(q.explanation_vi || q.explanation_zh || q.analysis_vi || q.analysis_zh || q.reason_vi || q.reason_zh || '').trim();
                        const readingExplanation = sectionType === 'reading' ? String(q.reading_explanation_vi || q.reading_explanation_zh || q.reading_explanation || part.explanation_vi || part.explanation_zh || '').trim() : '';
                        const detailText = readingExplanation || explanationText;

                        reviewHtml += `
                            <div style="background:#f8fafc;border-radius:16px;padding:16px;border:1.5px solid ${isCorrect ? '#bbf7d0' : '#fecaca'};margin-bottom:12px;box-shadow:0 2px 8px rgba(15,23,42,0.04);">
                                <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px;margin-bottom:8px;">
                                    <span style="font-weight:800;font-size:15px;color:#0f172a;">第${toChineseNumber(q.id)}题</span>
                                    <span style="background:${isCorrect ? '#f0fdf4' : '#fef2f2'};color:${isCorrect ? '#16a34a' : '#dc2626'};padding:3px 10px;border-radius:10px;font-weight:800;font-size:12px;border:1px solid ${isCorrect ? '#bbf7d0' : '#fecaca'};">
                                        ${isCorrect ? '✅ Đúng' : '❌ Sai'}
                                    </span>
                                </div>
                                ${q.question ? `<div style="font-size:14px;color:#334155;margin-bottom:8px;font-weight:600;line-height:1.6;">${q.question}</div>` : ''}
                                <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:6px;margin-bottom:10px;">
                                    ${(q.options || []).map(optStr => {
                                        ...
                                        const isRightOpt = letter === correctAns;
                                        const isUserOpt = letter === (userAns || '').toUpperCase();
                                        let bg = 'white';
                                        let color = '#334155';
                                        let border = '#cbd5e1';
                                        if (isRightOpt) { bg = '#dcfce7'; color = '#15803d'; border = '#86efac'; }
                                        else if (isUserOpt && !isRightOpt) { bg = '#fee2e2'; color = '#b91c1c'; border = '#fca5a5'; }
                                        return `<div style="padding:8px 12px;border-radius:8px;font-size:13px;font-weight:700;background:${bg};color:${color};border:1px solid ${border};">${optStr}</div>`;
                                    }).join('')}
                                </div>
                                <div style="display:grid;gap:8px;">
                                    <div style="background:white;border:1px solid #e2e8f0;border-radius:10px;padding:10px 12px;font-size:12.5px;color:#334155;line-height:1.5;">
                                        <div style="font-weight:800;color:#0284c7;margin-bottom:4px;">✅ Kết quả của bạn</div>
                                        <div><strong>Đáp án bạn chọn:</strong> ${userAns || 'Chưa chọn'}</div>
                                        <div><strong>Đáp án đúng:</strong> ${correctAns}</div>
                                    </div>
                                    ${detailText ? `<div style="background:#eff6ff;border-left:3px solid #3b82f6;padding:10px 12px;border-radius:8px;font-size:12.5px;color:#1e40af;line-height:1.6;">
                                        <strong>${sectionType === 'reading' ? '📖 Giải thích đoạn đọc' : '💡 Giải thích'}</strong><br>${detailText}
                                    </div>` : ''}
                                </div>
                            </div>
                        `;
            ...
            const resDivTemp = document.createElement('div');
            resDivTemp.innerHTML = reviewHtml;
            resContainer.querySelector('div:last-child').appendChild(resDivTemp);

            ci.appendChild(resContainer);
        }
...
function universalSearch() {
    ...
    function performSearch() {
        ...
        levels.forEach(levelId => {
            // Tìm trong Translation Module
            const transItems = window.translationDataByLevel ? window.translationDataByLevel[levelId] : null;
            if (transItems && Array.isArray(transItems)) {
                transItems.forEach((item, idx) => {
                    let content = ((item.chinese || '') + ' ' + (item.pinyin || '') + ' ' + (item.vietnamese || '') + ' ' + (item.hints ? item.hints.map(h => (typeof h === 'object' ? (h.cn + ' ' + h.vi + ' ' + h.py) : h)).join(' ') : '')).toLowerCase();
                    if (content.includes(query)) {
                        const key = `translation-${levelId}-item-${idx}`;
                        if (!resultsMap.has(key)) {
                            resultsMap.set(key, {
                                module: 'translation',
                                level: levelId,
                                itemIdx: idx,
                                title: `Luyện dịch (${levelId.toUpperCase()}): ${item.chinese || ''}`,
                                desc: `Dịch: ${item.vietnamese || ''}`,
                                moduleLabel: '🌐 Luyện dịch',
                                color: '#EC4899',
                                isTranslation: true
                            });
                        }
            ...
            modules.forEach(moduleId => {
                    ...
                    data.lessons.forEach(lesson => {
                        ...
                        if (moduleId === 'vocab') {
                            ...
                            vocabWords.forEach((word) => {
                                ...
                                const wordType = (word.word_type || '').toLowerCase();
                                ...
                                if (Array.isArray(word.examples)) {
                                    word.examples.forEach((ex) => {
                                        ...
                                        const pyText = typeof ex === 'object' ? (ex.py || ex.pinyin || '') : '';
                                        const viText = typeof ex === 'object' ? (ex.vi || ex.meaning || '') : '';
                                        if (cnText.toLowerCase().includes(query) || pyText.toLowerCase().includes(query) || viText.toLowerCase().includes(query)) {
                                            matchedInExample = cnText + (viText ? ` (${viText})` : '');
                                        }
                                    });
                                }

                                if (hanzi.includes(query) || pinyin.includes(query) || meaning.includes(query) || note.includes(query) || wordType.includes(query) || matchedInExample) {
                                    const key = `${moduleId}-${levelId}-word-${lesson.id}-${hanzi || Math.random()}`;
                                    if (!resultsMap.has(key)) {
                                        resultsMap.set(key, {
                                            module: moduleId,
                                            level: levelId,
                                            lessonId: lesson.id,
                                            title: `${word.hanzi || word.cn || ''} · ${word.meaning || word.vi || ''}`,
                                            desc: word.pinyin || word.py ? `${word.pinyin || word.py}` : '',
                                            moduleLabel: moduleLabels[moduleId] || moduleId,
                                            color: moduleColors[moduleId] || '#888',
                                            isVocabWord: true,
                                            wordHanzi: word.hanzi || word.cn || '',
                ...
                // Tìm trong items (hanzi, dictation, shadowing)
                if (data.items) {
            ...
            dropdown.querySelectorAll('.local-search-item').forEach(el => {
                ...
                el.onclick = function() {
                    ...
                    setTimeout(() => {
                        ...
                        if (targetCard) {
                            highlightAndScrollTarget(targetCard, `${m.wordHanzi || ''} ${m.wordMeaning || ''}`.trim());
                        }
                    }, 250);
                };
            });
...
window.retryLessonMistakesDirectly = async function(level, lessonId, moduleName, mistakeId) {
    ...
    if (mistakeId) {
        ...
        if (profile && Array.isArray(profile.wrongExercises)) {
            mistakeData = profile.wrongExercises.find(item => item.id === mistakeId) || null;
        }
    }
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            ...
            if (typeof window.resetLessonExercises === 'function') {
                window.resetLessonExercises(lessonEl);
            }

            let targetQuestion = lessonEl;
            if (mistakeData && mistakeData.question) {
                const needle = String(mistakeData.question || '').toLowerCase();
                const candidates = Array.from(lessonEl.querySelectorAll('.exercise-item'));
                const byIndex = candidates.find(item => String(item.dataset.exerciseIndex) === String(mistakeData.questionIdx));
                const byText = byIndex || candidates.find(item => {
                    const itemText = String(item.dataset.questionText || item.textContent || '').toLowerCase();
                    return itemText.includes(needle) || needle.includes(itemText);
                });
                if (byText) targetQuestion = byText;
            } else {
                targetQuestion = lessonEl.querySelector('.exercise-item, .exercise-card, .fill-item, .trans-item, .drag-drop-container, .match-container') || lessonEl;
            }

            targetQuestion.scrollIntoView({ behavior: 'smooth', block: 'center' });

            targetQuestion.style.transition = 'all 0.3s ease';
            targetQuestion.style.outline = '3px solid #ec4899';
            targetQuestion.style.outlineOffset = '4px';
            targetQuestion.style.boxShadow = '0 0 20px rgba(236,72,153,0.5)';
...
window.resetLessonExercises = function(containerEl) {
