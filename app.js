...
function renderExerciseItem(ex, idx, level, lessonId, totalCount, prefix = 'ex') {
    ...
    // ===== 4. TRANSLATION (DỊCH CÂU) =====
        ...
        ex.items.forEach((item, itemIdx) => {
            ...
            // (1) TỪ VỰNG GỢI Ý (SUGGESTED VOCABULARY HINTS: MAX 3 HINTS, 1 PER CLICK)
            const rawHints = item.hints || item.vocab || item.suggested_vocab || item.words || ex.hints || ex.vocab || null;
            let hintsList = [];
            if (Array.isArray(rawHints)) {
                hintsList = rawHints;
            } else if (typeof rawHints === 'string') {
                hintsList = rawHints.split(',').map(s => s.trim());
            }

            if (hintsList.length === 0 && (item.chinese || item.cn || item.question)) {
                hintsList = window.generateAutoHints(item.chinese || item.cn || item.question, item.pinyin || item.py, item.vietnamese || item.vi);
            }

            if (hintsList.length > 0) {
                const maxHints = Math.min(3, hintsList.length);
                const hintsBox = document.createElement('div');
                hintsBox.style.cssText = 'margin-bottom:12px;padding:10px 12px;background:#fff8fa;border:1px dashed #fbcfe8;border-radius:10px;font-size:12.5px;';
                
                const topRow = document.createElement('div');
                topRow.style.cssText = 'display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;margin-bottom:6px;';
                topRow.innerHTML = `<span style="font-weight:700;color:#be185d;">💡 Từ vựng gợi ý:</span>`;
                
                const hintBtn = document.createElement('button');
                hintBtn.type = 'button';
                hintBtn.style.cssText = 'padding:4px 12px;background:#fdf2f8;color:#be185d;border:1px solid #fbcfe8;border-radius:6px;font-size:11.5px;font-weight:700;cursor:pointer;';
                hintBtn.textContent = `💡 Hiển thị gợi ý từ vựng (0/${maxHints})`;
                topRow.appendChild(hintBtn);
                hintsBox.appendChild(topRow);

                const badgeContainer = document.createElement('div');
                badgeContainer.style.cssText = 'display:flex;flex-wrap:wrap;gap:6px;min-height:24px;align-items:center;';
                badgeContainer.innerHTML = `<span style="color:#94a3b8;font-size:11.5px;font-style:italic;">Nhấn nút trên để mở từ gợi ý (mỗi lần 1 từ, tối đa 3 gợi ý)</span>`;
                hintsBox.appendChild(badgeContainer);
                ...
                hintBtn.onclick = function(e) {
                    ...
                    if (revealed < maxHints) {
                        revealed++;
                        if (revealed === 1) badgeContainer.innerHTML = '';
                        const h = hintsList[revealed - 1];
                        const badge = document.createElement('span');
                        ...
                        badge.textContent = typeof h === 'object' ? `${h.cn || h.word || ''} (${h.py || ''}): ${h.vi || h.meaning || ''}` : h;
                        badgeContainer.appendChild(badge);

                        if (revealed < maxHints) {
                            hintBtn.textContent = `💡 Hiển thị gợi ý từ vựng (${revealed}/${maxHints})`;
                        } else {
                            hintBtn.textContent = `💡 Hiển thị gợi ý từ vựng (${revealed}/${maxHints} - Đã hết)`;
...
function prevVocabTab(hskLevel, lessonNum) {
    ...
    if (idx > 0) {
        ...
        // ================================================================
        // RENDER GRAMMAR LESSONS & GLOBAL SHOW LESSON HANDLERS
        // ================================================================
...
item.onclick = function() {
    ...
    (async function() {
        ...
        setTimeout(() => {
            ...
            // Find, scroll to, and highlight detailed searched content
            setTimeout(() => {
                ...
                if (lessonEl) {
                    let matchedEl = null;
                    const searchTerm = (queryTerm || searchTitle).toLowerCase();
                    const candidates = lessonEl.querySelectorAll('.vocab-card, .trans-mod-card, .trans-item, .exercise-card, .fill-item, .grammar-tab-content, .subcard, p, h3, h4');
                    
                    for (const cand of candidates) {
                        const txt = (cand.textContent || '').toLowerCase();
                        if (txt.includes(searchTerm) && searchTerm.length > 0) {
                            matchedEl = cand.closest('.vocab-card, .trans-mod-card, .trans-item, .exercise-card, .fill-item, .grammar-tab-content') || cand;
                            break;
                        }
                    }
                    
                    const targetEl = matchedEl || lessonEl;
                    ...
                    toast.innerHTML = `🔍 Chi tiết từ tìm kiếm: <b>${searchTitle}</b>`;
...
window.renderTranslationModule = function(level, data) {
        ...
        <div style="background:linear-gradient(135deg,#fdf2f8,#fce7f3);padding:24px;border-radius:20px;margin-bottom:24px;border:2px solid #fbcfe8;box-shadow:0 8px 24px rgba(236,72,153,0.1);">
            <div style="display:flex;align-items:center;gap:12px;margin-bottom:8px;">
                ...
                <div>
                    ...
                    <p style="font-size:13.5px;color:#9d174d;margin:4px 0 0 0;">Thực hành dịch câu mẫu chuẩn HSK với nút mở từng từ vựng gợi ý (tối đa 3 từ) & chấm điểm Đúng (Xanh) - Sai (Đỏ)</p>
                </div>
            </div>
        </div>
    ...
    items.forEach((item, idx) => {
        ...
        const totalHintsCount = Math.min(3, hints.length);
        ...
        html += `
            <div class="trans-mod-card" id="trans-mod-item-${lvl}-${idx}" style="background:white;padding:20px;border-radius:16px;border:1.5px solid #fbcfe8;box-shadow:0 4px 14px rgba(0,0,0,0.03);">
                ...
                <!-- (1) TỪ VỰNG GỢI Ý: ĐỒNG BỘ HOÀN TOÀN VỚI BÀI TẬP BÀI HỌC -->
                ${totalHintsCount > 0 ? `
                <div style="margin-bottom:12px;padding:10px 12px;background:#fff8fa;border:1px dashed #fbcfe8;border-radius:10px;font-size:12.5px;">
                    <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;margin-bottom:6px;">
                        <span style="font-weight:700;color:#be185d;">💡 Từ vựng gợi ý:</span>
                        <button id="hint-btn-${lvl}-${idx}" onclick="window.revealTranslationHint('${lvl}', ${idx})" 
                                style="padding:4px 12px;background:#fdf2f8;color:#be185d;border:1px solid #fbcfe8;border-radius:6px;font-size:11.5px;font-weight:700;cursor:pointer;">
                            💡 Hiển thị gợi ý từ vựng (0/${totalHintsCount})
                        </button>
                    </div>
                    <div id="hints-box-${lvl}-${idx}" data-revealed="0" style="display:flex;flex-wrap:wrap;gap:6px;min-height:24px;align-items:center;">
                        <span style="color:#94a3b8;font-size:11.5px;font-style:italic;">Nhấn nút trên để mở từ gợi ý (mỗi lần 1 từ, tối đa 3 gợi ý)</span>
                    </div>
                </div>
                ` : ''}
...
window.renderEntertainmentModule = function(level, categoryFilter) {
    ...
    const entertainmentItems = [
        ...
        },
        {
            ...
            level: 'hsk2',
            category: 'music',
            categoryLabel: '🎵 Nhạc & Song',
            title: 'Bài Hát: Ngọt Ngào (甜蜜蜜 - Đặng Lệ Quân)',
            chineseTitle: '甜蜜蜜 (Tián Mì Mì)',
            synopsis: 'Ca khúc bất hủ tiếng Trung với giai điệu nhẹ nhàng, từ vựng vô cùng trong sáng và dễ hát theo.',
            embedUrl: 'https://www.youtube.com/embed/3y3FhEfl3X8',
            vocab: [
    ...
    let html = `
        <div style="background:linear-gradient(135deg,#fef3c7,#fde68a);padding:24px;border-radius:20px;margin-bottom:24px;border:2px solid #fcd34d;box-shadow:0 8px 24px rgba(245,158,11,0.12);">
            ...
            <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:16px;">
                ...
                <button onclick="window.renderEntertainmentModule('${lvl}', 'music')" style="padding:7px 16px;border-radius:20px;border:none;font-weight:700;font-size:12.5px;cursor:pointer;${cat==='music'?'background:#d97706;color:white;':'background:white;color:#92400e;border:1px solid #fcd34d;'}">🎵 Nhạc & Song</button>
            </div>
        </div>
...
window.retryLessonMistakesDirectly = async function(level, lessonId) {
    ...
    if (typeof window.showContent === 'function') {
        await window.showContent('grammar', targetLvl);
    }

    setTimeout(() => {
        if (typeof window.showLesson === 'function') {
            window.showLesson(targetLvl, cleanId, '999');
        } else if (typeof window.showVocabLesson === 'function') {
        ...
        setTimeout(() => {
            ...
            const targetQuestion = lessonEl.querySelector('.exercise-card, .fill-item, .trans-item, .drag-drop-container') || lessonEl;
...
window.resetLessonExercises = function(containerEl) {
    ...
    containerEl.querySelectorAll('input[type="radio"]').forEach(radio => {
        ...
        if (radio.parentElement) {
            ...
            radio.parentElement.style.background = '#f8fafc';
            radio.parentElement.style.borderColor = '#f1f5f9';
            radio.parentElement.style.color = '#1e293b';
            radio.parentElement.style.fontWeight = 'normal';
        }
    });

    containerEl.querySelectorAll('input[type="text"]').forEach(input => {
        input.value = '';
        input.style.borderColor = '#cbd5e1';
        input.style.background = '#ffffff';
    });

    containerEl.querySelectorAll('.result-badge').forEach(b => b.remove());
    containerEl.querySelectorAll('.exercise-result, .fill-result, .trans-result').forEach(div => {
